...
def cases_on_a_given_day(date):
    ...
    covid_data = covid_data.groupby(["areaName"], as_index=False)[[ "newCasesBySpecimenDate-0_59", "newCasesBySpecimenDate-60+" ]].sum()
    
    if len(covid_data) > 1:
        covid_data_df = covid_data.sort_values([ "newCasesBySpecimenDate-0_59", "newCasesBySpecimenDate-60+" ],ascending=False)[:20] 
        population_pyramid(title, covid_data_df, covid_data_df["newCasesBySpecimenDate-0_59"], covid_data_df["newCasesBySpecimenDate-60+"], covid_data_df["areaName"], x_label = "Cases count", y_label ="Age group")
    else:
        return messagebox.showinfo("showinfo", "No data available, select again")
...
def cases_per_month_over_a_period(date_one, date_two):
    ...
    if len(covid_data_new) > 1:
        ...
        tree_map_visual(title, covid_data_new["newCasesBySpecimenDate-Sum"], labels )
    else:
        return messagebox.showinfo("showinfo", "No data available, select again")
...
def areas_with_highest_cases_on_a_given_day(date_selected):  
    ...
    if len(covid_data) > 1:
        diverging_text_visuals(title, covid_data, "PercentageChange-cases_Sum" )
    else:
        return messagebox.showinfo("showinfo", "No data available, select again")
...
def compare_two_areas_per_day(region_one, region_two, date_selected):
    ...
    if len(covid_data) > 1:
        grouped_bar_chart_visual(title, covid_data, "areaName", "Covid cases", ["Age Group 0-59", "Age Group 60+", "Age Group Sum"])
    else:
        return messagebox.showinfo("showinfo", "No data available, select again")
